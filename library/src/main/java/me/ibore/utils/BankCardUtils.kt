...
object BankCardUtils {
     ...
     */
    fun check(bankCard: String?): Boolean {
        return if (bankCard == null || bankCard.trim { it <= ' ' }.isEmpty()) false else try {
            val bit = getVerifyCode(bankCard.substring(0, bankCard.length - 1))
            if (bit == 'N') false else bankCard[bankCard.length - 1] == bit
        } catch (e: Exception) {
            LogUtils.d(e)
            false
        }
     ...
     * @return 银行卡校验位
     */
    fun getVerifyCode(bankCard: String?): Char {
        return try {
            if (bankCard == null || bankCard.trim { it <= ' ' }
                    .isEmpty() || !bankCard.matches(Regex("\\d+"))) {
                // 如果传的不是数据返回 N
                return 'N'
            }
            val chs = bankCard.trim { it <= ' ' }.toCharArray()
            var luhnSum = 0
            val len = chs.size
            var i = len - 1
            var j = 0
            ...
            if (luhnSum % 10 == 0) '0' else (10 - luhnSum % 10 + '0'.toInt()).toChar()
        } catch (e: Exception) {
            LogUtils.d(e)
            'N'
        }
     ...
     * @return 银行开户行及卡种
     */
    fun getBankName(bankCard: String?): String {
        return if (bankCard == null || bankCard.trim { it <= ' ' }.length < 6) "" else try {
            // 通过银行卡的前六位确定
            val cardBin6 = bankCard.trim { it <= ' ' }.substring(0, 6)
            var index = -1
            var i = 0
            val len = BANK_BIN.size
            while (i < len) {
                if (cardBin6 == BANK_BIN[i]) {
                    index = i
                }
                i++
            }
