...
object ShadowUtils {
    ...
    class Config {
        ...
        fun apply(src: Drawable?): Drawable {
            var srcTemp = src
            if (srcTemp == null) {
                srcTemp = ColorDrawable(Color.TRANSPARENT)
            }
            val drawable = StateListDrawable()
            drawable.addState(
                intArrayOf(android.R.attr.state_pressed),
                ShadowDrawable(
                    srcTemp, shadowRadius, shadowSizeNormal,
            ...
            drawable.addState(
                StateSet.WILD_CARD,
                ShadowDrawable(
                    srcTemp, shadowRadius, shadowSizePressed,
    ...
    class ShadowDrawable(
        ...
        private val mShadowEndColor: Int = mShadowStartColor and 0x00ffffff
        ...
        fun setShadowSize(shadowSize: Float, maxShadowSize: Float) {
            var shadowSizeTemp = shadowSize
            var maxShadowSizeTemp = maxShadowSize
            require(!(shadowSizeTemp < 0 || maxShadowSizeTemp < 0)) { "invalid shadow size" }
            shadowSizeTemp = toEven(shadowSizeTemp).toFloat()
            maxShadowSizeTemp = toEven(maxShadowSizeTemp).toFloat()
            if (shadowSizeTemp > maxShadowSizeTemp) {
                shadowSizeTemp = maxShadowSizeTemp
            }
            if (mRawShadowSize == shadowSizeTemp && mRawMaxShadowSize == maxShadowSizeTemp) {
                return
            }
            mRawShadowSize = shadowSizeTemp
            mRawMaxShadowSize = maxShadowSizeTemp
            mShadowSize = Math.round(shadowSizeTemp * mShadowMultiplier).toFloat()
            mMaxShadowSize = maxShadowSizeTemp
            mDirty = true
            invalidateSelf()
        }
        ...
        var cornerRadius: Float
            ...
            set(radius) {
                ...
                radiusTemp = radiusTemp.roundToInt().toFloat()
                if (mCornerRadius == radiusTemp) {
                    return
                }
                mCornerRadius = radiusTemp
                mDirty = true
                invalidateSelf()
            }
        ...
        init {
            ...
            mCornerRadius = radius.roundToInt().toFloat()
