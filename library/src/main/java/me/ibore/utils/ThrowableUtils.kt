...
object ThrowableUtils {
    ...
    fun getFullStackTrace(throwable: Throwable?): String {
        var throwableTemp = throwable
        val throwableList: MutableList<Throwable> = ArrayList()
        while (throwableTemp != null && !throwableList.contains(throwableTemp)) {
            throwableList.add(throwableTemp)
            throwableTemp = throwableTemp.cause
        }
